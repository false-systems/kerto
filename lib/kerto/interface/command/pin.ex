defmodule Kerto.Interface.Command.Pin do
  @moduledoc """
  Pins a node or relationship so it never decays or gets pruned.
  """
  ...
  def execute(engine, args) do
    ...
    cond do
      not is_nil(node) ->
        pin_node(engine, node, args)

      not is_nil(source) and not is_nil(relation) and not is_nil(target) ->
        pin_relationship(engine, source, relation, target, args)
  ...
  defp pin_node(engine, node, args) do
    case Validate.node_kind(Map.get(args, :kind, :file)) do
      {:ok, kind} ->
        case Kerto.Engine.pin_node(engine, kind, node) do
  ...
  defp pin_relationship(engine, source, relation, target, args) do
    with {:ok, source_kind} <- Validate.node_kind(Map.get(args, :source_kind, :file)),
      ...
      case Kerto.Engine.pin_relationship(
             engine,
             source_kind,
             source,
             relation_atom,
